...
{
    public class BestNodeSearchStrategy : IStrategy
    {
        ...
        public BestNodeSearchStrategy(int maxDepth = 6)
        {
            this.maxDepth = maxDepth;
        }

        private static readonly int[] lineScore = new int[64];
        static BestNodeSearchStrategy()
        {
        ...
        {
            return (int)((long)alpha + ((long)beta - (long)alpha) * ((long)count - (long)1) / (long)count);
        }

        public void MakeMove(BoardProxy board)
        {
            UltimateTicTacToe ttt = board.GetBoardCopy();
            int alpha = -int.MaxValue, beta = int.MaxValue;
            List<int> moves = ttt.GetAllMovesId().ToList();
            while (alpha + 1 < beta && moves.Count > 1)
            {
                int guess = NextGuess(alpha, beta, moves.Count);
                List<int> newMoves = new List<int>();
                foreach (int move in moves)
                {
                    ttt.MakeMove(move);
                    int value = -AlphaBeta(ttt, maxDepth, - guess, -guess + 1).score;
                    ttt.Undo();
                    if (value >= guess)
                    {
                        newMoves.Add(move);
